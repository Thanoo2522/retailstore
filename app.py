...
def register_shop():
    ...
    db.collection(shopname).document("register").set({
        "shopname": shopname,
        "phone": phone,
        "password": password
    })
...
def save_order():
    try:
        ...
        # ===============================
        # 4️⃣ เพิ่มสินค้า → items (สร้างใหม่ทุกครั้ง)
        # ===============================
        item_ref = (
            order_ref
              .collection("items")
              .document()  # 🔥 auto id → item ใหม่ทุกครั้ง
        )
